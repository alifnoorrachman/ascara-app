<div class="container py-10 w-full max-w-3xl px-4 sm:px-6 lg:px-8 bg-colorNetral rounded-xl">
    ...
    <div class="space-y-4 py-4">
        @foreach($faqs as $faq)
        <div class="accordion-item text-colorOne bg-white shadow">
            <h2 class="accordion-header">
                <button 
                    class="accordion-button text-left w-full py-4 px-5 rounded-lg bg-blue-100 font-medium hover:bg-colorOne hover:text-colorNetral flex justify-between items-center"
            ...
            <div id="collapse{{ $faq->id }}" class="accordion-collapse max-h-0 overflow-hidden transition-all duration-300 ease-in-out">
                <div class="accordion-body py-4 px-5 text-gray-700 text-base rounded-b-lg bg-white">
                    {{ $faq->answer }}
                </div>
            </div>
        </div>
