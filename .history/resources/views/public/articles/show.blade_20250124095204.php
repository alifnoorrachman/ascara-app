...
<div class="container mx-auto px-2 my-4 sm:px-6 lg:px-8">
    <article class="max-w-7xl mx-auto rounded-xl shadow-lg p-6 sm:p-8 mb-12">
        {{-- Article Header with Date and Share --}}
        <div class="flex justify-between items-center text-sm mb-4">
            <div>{{ $article->created_at->format('l, d F Y') }}</div>
        ...
        @else
        <div class="w-full h-64 sm:h-80 mb-6 rounded-xl"></div>
        @endif
        ...
        {{-- Article Content --}}
        <div class="prose prose-lg max-w-none leading-relaxed p-6">
            {!! $article->content !!}
        </div>
    </article>
</div>
...
<div class="max-w-6xl mx-auto py-8">
    ...
    <div data-animate>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
            ...
            <x-article-card 
                :image="$relatedArticle->image ? Storage::url($relatedArticle->image) : null"
                :title="$relatedArticle->title"
                :date="$relatedArticle->created_at->format('l, d F Y')"
                :link="route('articles.show', ['slug' => $relatedArticle->slug])"
            />
            @endforeach
        </div>
    </div>
