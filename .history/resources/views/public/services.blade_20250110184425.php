...
{{-- Hero --}}
<section class="parallax-section mx-2 rounded-xl py-44 px-6 sm:px-8 lg:px-20 bg-colorOne text-colorNetral">
...
{{-- Card Service --}}
<section data-animate class="parallax bg-colorNetral pt-12 px-4 sm:px-8 lg:px-16 xl:px-20 text-colorThree">
    <!-- Menggunakan Grid -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach($services as $service)
            <div class="bg-white p-8 rounded-xl shadow-lg hover:shadow-2xl hover:scale-105 transition duration-300 flex flex-col h-full">
                <!-- Ikon di Dalam Card -->
                <div class="bg-colorThree text-white p-3 rounded-lg inline-block self-start">
                    <i class="fas fa-cube text-lg"></i> <!-- Sesuaikan ikon jika perlu -->
                </div>
                ...
                <!-- Tags (diletakkan dalam div fleksibel agar tetap di dalam card) -->
                <div class="mt-4 flex flex-wrap gap-2">
                    <span class="px-3 py-1 text-xs font-semibold text-colorThree border border-colorThree rounded-full">
                        AUTOMATION
                    </span>
                    <span class="px-3 py-1 text-xs font-semibold text-colorThree border border-colorThree rounded-full">
                        CUSTOM API
