...
{{-- Card Service --}}
{{-- Card Service --}}
<section data-animate class="parallax pt-12 px-4 sm:px-8 lg:px-16 xl:px-20">
    <!-- Menggunakan Grid -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach($services as $service)
            {{-- Tambahkan link wrapper di sini --}}
            <a href="{{ route('services.show', $service->id) }}" class="block">
                <div class="p-8 rounded-xl shadow-lg hover:shadow-2xl hover:scale-105 transition duration-300 flex flex-col h-full">
                    <!-- Ikon di Dalam Card -->
                    <div class="p-3 rounded-lg inline-block self-start">
                    ...
                    <div class="flex flex-col flex-grow mt-4">
                        ...
                        <p class=" text-base leading-relaxed flex-grow">{{ $service->description }}</p>
                    </div>

                    <!-- Learn More -->
                    <p class="mt-4 text-sm hover:underline font-semibold">
                        Learn More
                    </p>
                </div>
            </a>
