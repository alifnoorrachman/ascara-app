            ...
            <div class="flex flex-wrap justify-center gap-4">
                ...
                @foreach($services as $service)
                    <a href="{{ route('works', ['service' => $service->id]) }}" 
                       class="px-4 py-2 {{ request('service') == $service->id ? 'bg-colorNetral text-colorOne' : 'border-1 border-colorThree text-colorThree' }} rounded-md transition-colors duration-300">
                        {{ $service->title }}
                    </a>
                @endforeach
            </div>
