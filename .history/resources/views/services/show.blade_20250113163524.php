...
@section('content')
{{-- Hero Section --}}
<section class="parallax-section mx-2 rounded-xl py-32 px-6 sm:px-8 lg:px-20 bg-colorNetral text-colorOne">
    <div class="container mx-auto">
        <p class="text-xl mb-4 text-center sm:text-lg md:text-xl lg:text-2xl leading-relaxed">
            {{ $service->subtitle ?? 'Detailed overview of our service' }}
        </p>
        <h1 class="text-center pt-2 mb-8 text-4xl sm:text-5xl md:text-6xl lg:text-8xl font-bold leading-tight">
            {{ $service->title }}
        </h1>
    </div>
</section>
...
<section data-animate class="parallax bg-colorOne py-20 px-4 sm:px-8 lg:px-16 xl:px-20 text-colorNetral">
    <div class="container mx-auto">
        ...
        {{-- Process Steps --}}
        <div class="mb-20">
            <h2 class="text-3xl font-bold text-gray-800 mb-12 text-center">Our Process</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach($service->process ?? [] as $step)
                    <div class="bg-white p-8 rounded-xl shadow-lg">
                        <div class="bg-colorOne text-white w-12 h-12 rounded-full flex items-center justify-center mb-6">
                            {{ $loop->iteration }}
                        </div>
        ...
        @if(isset($relatedServices) && count($relatedServices) > 0)
            <div class="mb-20">
                <h2 class="text-3xl font-bold text-gray-800 mb-12 text-center">Related Services</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
