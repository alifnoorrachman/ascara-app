...
<section data-animate class="parallax bg-colorOne py-0 px-4 sm:px-8 lg:px-16 xl:px-20 text-colorNetral">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <article>
            {{-- Service Overview --}}
            <div class="bg-white rounded-xl shadow-lg p-6 sm:p-8 mb-12">
                
                {{-- Image Section --}}
                <div class="mb-6">
                    <img src="{{ asset('storage/' . $service->image) }}" 
                         alt="{{ $service->title }}" 
                         class="w-full h-64 sm:h-80 md:h-96 rounded-xl object-cover">
                </div>
                ...
                {{-- Content Section --}}
                <div class="prose prose-lg max-w-none text-gray-700 leading-relaxed p-6">
                    {!! $service->content !!}
                </div>
            </div>
        </article>
