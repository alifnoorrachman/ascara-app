...
<section data-animate class="parallax py-0 px-4 sm:px-8 lg:px-16 xl:px-20">
    <div class="container max-w-7xl my-4">
        <article>
        {{-- Service Overview --}}
        <div class="rounded-xl shadow-lg p-2 mb-12">
            ...
            {{-- Content Section --}}
                <div class="prose prose-lg max-w-none mb-8 p-8">
                    {!! $service->content !!}
                </div>
            </div>
        </article>
        ...
        @if(isset($relatedServices) && count($relatedServices) > 0)
            <div class="mb-20">
                <h2 class="text-2xl font-medium mb-12 text-start">Related Services</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($relatedServices as $relatedService)
                        <div class=" p-8 rounded-xl shadow-lg hover:shadow-2xl hover:scale-105 transition duration-300">
                            <div class=" p-3 rounded-lg inline-block">
                                <i class="fas fa-cube text-lg"></i>
                            </div>
                            <h3 class="text-2xl font-bold mt-4 mb-2">{{ $relatedService->title }}</h3>
                            <p class=" mb-4">{{ Str::limit($relatedService->description, 100) }}</p>
